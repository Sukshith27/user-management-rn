...
type EditUserScreenProps = {
  user: User;
  onClose: () => void;
  onSave: (updated: { firstName: string; lastName: string; email: string; role: string }) => void;
  onDelete: () => void;
};

export default function EditUserScreen({ user, onClose, onSave, onDelete }: EditUserScreenProps) {
  const [firstName, setFirstName] = useState(user.name.split(' ')[0] || '');
  const [lastName, setLastName] = useState(user.name.split(' ')[1] || '');
  const [email, setEmail] = useState(user.email || '');
  const [role, setRole] = useState<'Admin' | 'Manager'>(user.type as 'Admin' | 'Manager');
  const [error, setError] = useState<string | null>(null);
  const [firstNameError, setFirstNameError] = useState(false);
  const [lastNameError, setLastNameError] = useState(false);
  const indicator = useRef(new Animated.Value(ROLES.indexOf(user.type) * TAB_WIDTH)).current;
  ...
  function handleRoleTab(idx: number) {
    ...
    <View style={styles.modalBg}>
      <View style={styles.container}>
        ...
        <View style={styles.headerRow}>
          <Text style={styles.headline}>Edit User</Text>
        ...
        <TouchableOpacity style={styles.createBtn} onPress={handleSave}>
          <Text style={styles.createBtnText}>Edit User</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
